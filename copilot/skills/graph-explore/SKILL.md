...
Call the selected MCP tools directly. They are available as `mcp__graphify__<tool_name>` in
this session — verify that this prefix matches your Copilot MCP configuration. If the tools
are not found under this prefix, check your Copilot settings for the correct MCP tool prefix
for the `graphify` server and adjust accordingly.

Do not call more than 3 tools. Do not dump raw output.
