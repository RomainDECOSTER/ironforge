...
description: >
  ...
  the Copilot CLI integration, installs git hooks for incremental rebuild, and adds
...
## Step 3 — Platform setup (Copilot CLI integration)
...
**Note for Copilot CLI users**: `graphify install` writes its `PreToolUse` hook to the
Claude Code `settings.json` path. On Copilot CLI, the settings path may differ — after
running this command, verify that the PreToolUse hook is registered in your Copilot
settings file. If it is not, copy the hook entry from `settings.json` into your Copilot
equivalent settings file manually.
...
✓ Graphify installed
✓ Copilot CLI integration configured  (graphify install — verify PreToolUse hook is active)
